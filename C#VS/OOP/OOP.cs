...
namespace OOP // Если namespace (названия) одинаковые в файлах, то они позволяют юзать классы из других файлов
{
	class Program
	{
        ...
        static void Main(string[] args)
		{
            ...
            // (частичный класс) partial класс создаётся через модификатор partial
            // ФАЙЛ Student.cs
            // Этот модификатор позволяет разбивать файл(класс) на несколько частей
            // Через этот модификатор делают разные фичи в приложениях, например
            // С методами также работает почти



            //// const vs readonly                                                      const vs readonly
            // const - не даёт менять переменную более никогда
            // Такое поле всегда требует значение
            // Обычно константы пишут полностью через КАПС
            // НЕЯВНО СТАТИЧЕСКИЕ !!!!

            // readonly
            // можно не присваивать значение
            // Не является неявно статичным, но можно сделать явно статическим
            // После того как данные через конструктор поместили, то уже изменить нельзя будет



            //// СИНТАКСИС ИНИЦИАЛИЗАЦИИ ОБЪЕКТОВ КЛАССА                                СИНТАКСИС ИНИЦИАЛИЗАЦИИ ОБЪЕКТОВ КЛАССА
            //ФАЙЛ Student.cs
            //Student student1 = new Student();
            //student1.Age = 22;
            //student1.Name = "Robin";

            //Student student2 = new Student // ВНИМАНИЕ НА СКОБКИ (ОНИ ФИГУРНЫЕ)
            //{
            //    Age = 5,
            //    Name = "Anna"
            //}; //<-- semicolon тут
            // Особо разницы между первым и 2 способами нет. Только то, что
            // во втором способе присоздании экземпляра присваивается, а не после



            //// НАСЛЕДОВАНИЕ                                                           НАСЛЕДОВАНИЕ
            // ФАЙЛ Student.cs
            // В проект можно добавлять Class Diagram через Add - New Item
            // Множественное наследование классов в C# нет (к интерфесам не относится)
            // Тип нельзя через запятую Studik : Person, MyClass (так нельзя)
        ...
        // }
            ...
            //ФАЙЛ Car.cs
            // мы можем использовать protected поля (приватные не можем)
